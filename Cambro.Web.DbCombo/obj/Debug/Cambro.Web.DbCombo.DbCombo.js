...
function DbComboTextBoxKeyUp(c){
	...
	{//Run query.
		...
		if (eval(_c+"_lastQuery")!=document.all[c+"_ulbQueryHidden"].value){
...
function DbComboTextBoxKeyDown(c){
	...
	var _c=c.replace(/\$/g,"_");
	var dropDown = document.all[c+"_ulbDropDown"];
	var resultsSpan = document.all[_c+"_ulbResultsSpan"];
...
function DbComboSendRequestIfIdle(c,keyPressNumber){
	...
	if ( eval(_c+"_keyPressNumber") == keyPressNumber &! eval(_c+"_abortLookup") ){
		DbComboShowResults(c,false);
		eval(_c+"_lastQuery=document.all[\""+c+"_ulbQueryHidden\"].value;");
		eval(_c+"_numRows="+_c+"_initialNumRows;");
		eval(_c+"_skipRecords=0;");
		eval(_c+"_keyPressNumber=0;");
		DbComboSendRequest(c,true,true,false);
...
function DbComboSendRequest(c,clearlist,setFocus,reQuery){
	...
	try{
		...
		queryTag.text=document.all[c+"_ulbQueryHidden"].value;
	...
	try
	{
		if (!eval(_c+"_abortLookup"))
		{
			...
			{//all xml has been received.
				...
				var tmpSelectedIndex = document.all[c+"_ulbDropDown"].selectedIndex;
				if (clearlist){
					DbComboClearSelect(c);
				}
				...
				else
				{
					...
					if (count!=0)
					{
						if (document.all[c+"_ulbDropDown"].length<=1){
							document.all[c+"_ulbDropDown"].size=2;
						}
						else if (document.all[c+"_ulbDropDown"].length<eval(_c+"_initialNumRows")){
							document.all[c+"_ulbDropDown"].size=document.all[c+"_ulbDropDown"].length;
						}
						else{
							document.all[c+"_ulbDropDown"].size=eval(_c+"_initialNumRows");
						}
					}
					...
					{
						if(setFocus)
							document.all[c+"_ulbTextBox"].focus();
						
						if (!clearlist &! QueryFoundInStream &! ValueFoundInStream)
							document.all[c+"_ulbDropDown"].selectedIndex = tmpSelectedIndex;
...
function DbComboResizeResults(c,hideSelectTags){
	...
	var height = parseInt(drop.clientHeight)-3;
